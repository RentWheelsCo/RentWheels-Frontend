...
function readCachedProfile() {
  try { return JSON.parse(sessionStorage.getItem("rw_profile") || "null"); } catch { return null; }
}

function writeCachedProfile(profile) {
  try { sessionStorage.setItem("rw_profile", JSON.stringify(profile || null)); } catch {}
}
...
function logout() {
  ...
  localStorage.removeItem("rw_notifications");
  window.location.href = "../html/login.html";
}

function getNotifications() {
  try { return JSON.parse(localStorage.getItem("rw_notifications") || "[]"); } catch { return []; }
}

function setNotifications(items) {
  try { localStorage.setItem("rw_notifications", JSON.stringify(Array.isArray(items) ? items : [])); } catch {}
}
...
function renderHeader(activeKey) {
  ...
  const navHtml = navItems.map((item) => {
    ...
    return `<a href="${item.href}" class="rw-nav-link${isActive ? " active" : ""}" data-nav="${item.key}" data-protected="${item.protected}" ${isActive ? 'aria-current="page"' : ""}>${item.label}</a>`;
  }).join("");
  ...
  const notifItemsHtml = notifCount === 0
    ...
    : notifications.map((n) => {
      ...
      const message = escapeHtml(n?.message || "No additional details.");
      const type = escapeHtml(n?.type || "GENERAL");
      const timeText = escapeHtml(formatNotifTime(n?.createdAt));
      const canMark = Number.isInteger(id) && id > 0;
      return `
        <div class="rw-notif-panel__item" data-notification-id="${canMark ? id : ""}">
          <button class="rw-notif-toggle" type="button" ${canMark ? `data-toggle-notif="${id}"` : ""}>
            <span class="rw-notif-title">${title}</span>
            <span class="rw-notif-chevron">?</span>
          </button>
          <div class="rw-notif-details">
            <div class="rw-notif-panel__message">${message}</div>
            <div class="rw-notif-meta"><span>Type: ${type}</span><span>${timeText}</span></div>
            <button class="rw-notif-action"${canMark ? ` data-mark-read="${id}"` : " disabled"} type="button">Mark as read</button>
          </div>
        </div>
      `;
    }).join("");
  ...
  const notificationBellHtml = `
    <div class="rw-notification-wrap">
      <button class="rw-notification" aria-label="Notifications" aria-expanded="false" type="button" id="rw-notification-btn">
        <svg width="20" height="20" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" aria-hidden="true"><path d="M12 22c1.1 0 2-.9 2-2h-4a2 2 0 002 2zm6-6v-5c0-3.07-1.64-5.64-4.5-6.32V4a1.5 1.5 0 00-3 0v.68C7.63 5.36 6 7.92 6 11v5l-2 2v1h16v-1l-2-2z" fill="currentColor"/></svg>
        <span class="rw-notification-badge${notifCount === 0 ? " hidden" : ""}" id="rw-notif-badge" aria-label="${notifCount} notifications">${notifCount}</span>
      </button>
      <div class="rw-notif-panel" id="rw-notif-panel" role="dialog" aria-label="Notifications panel">
        <div class="rw-notif-panel__header"><span>Notifications</span><button id="rw-mark-all-read-btn" class="rw-notif-action rw-notif-action--all" type="button"${notifCount === 0 ? " disabled" : ""}>Mark all as read</button></div>
  ...
  const authActionHtml = isAuthed()
    ...
    : `<button type="button" class="rw-btn-login" id="rw-login-btn" aria-label="Log in">Log in</button>`;

  return `
    <header class="rw-header">
      <a class="rw-brand" href="../html/home.html" aria-label="RentWheels home"><img src="${ASSETS.brandLogo}" alt="RentWheels logo" /></a>
      <nav class="rw-nav" aria-label="Primary navigation">${navHtml}</nav>
      <div class="rw-actions">
        ${notificationBellHtml}
        <a class="rw-dashboard-link${activeKey === "dashboard" ? " active" : ""}" href="../html/dashboard.html" id="rw-dashboard-link" data-protected="true" aria-label="Dashboard">Dashboard</a>
        ${authActionHtml}
      </div>
    </header>
  `;
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const rerenderHeader = () => {
    ...
    headerTarget.innerHTML = renderHeader(pageKey);
    bindHeaderEvents();
  };
  ...
  function bindHeaderEvents() {
    ...
    const markAllBtn = document.getElementById("rw-mark-all-read-btn");
    if (markAllBtn) {
      markAllBtn.addEventListener("click", async (e) => {
        ...
        try {
          await window.RW_API?.notifications?.markAllRead?.();
          setNotifications([]);
          rerenderHeader();
        } catch {
    ...
    document.querySelectorAll("[data-mark-read]").forEach((btn) => {
      btn.addEventListener("click", async (e) => {
        e.stopPropagation();
        const id = Number(btn.getAttribute("data-mark-read"));
        if (!Number.isInteger(id) || id <= 0 || btn.disabled) return;
        btn.disabled = true;
        try {
          await window.RW_API?.notifications?.markRead?.(id);
          const next = getNotifications().filter((n) => Number(n?.id) !== id);
    ...
    document.querySelectorAll(".rw-notif-toggle").forEach((btn) => {
      btn.addEventListener("click", (e) => {
        e.stopPropagation();
        const item = btn.closest(".rw-notif-panel__item");
        if (!item) return;
  ...
  window.RW_API?.auth?.profile?.()
    ...
    .then((n) => {
      ...
      const mapped = rows.map((r) => ({ id: r.id, title: r.title || "", message: r.message || "", type: r.type || "", createdAt: r.createdAt || "" }));
  ...
  document.addEventListener("click", (e) => {
    ...
    const panel = document.getElementById("rw-notif-panel");
    const btn = document.getElementById("rw-notification-btn");
    if (wrap && panel && btn && !wrap.contains(e.target)) {
